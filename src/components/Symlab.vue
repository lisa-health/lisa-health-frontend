                ...
                <v-window v-model="step">
                  <!--window  1-->
                    <v-window-item :value="1">
                        <v-container  fluid >
                            <v-layout  row wrap>
                                <v-flex v-for="(i,index) in icons"  :key="index" xs4 justify-center align-self-center @click="toSecond(index)">
                    ...
                    <v-window-item :value="2">
                        <v-container  fluid >
                            <v-layout  row wrap>
                                <v-flex v-for="(s,i) in second"  :key="i" xs4 justify-center align-self-center @click="toSymList(s)">
                                    <v-btn block flat large disabled>
                                        <svg class="icon" aria-hidden="true" font-size="36px" >
                                            <use  :xlink:href='"#"+(deIcons[first])[i]'></use>
                                        </svg>
                                    </v-btn>
                                    <p class="text-xs-center">{{s}}</p>
                                </v-flex>
                            </v-layout>
                        </v-container>
                    </v-window-item>
...
export default {
    ...
    methods: {
        ...
        toSecond(index){
            this.first=index
            this.second = this.secondDepartment[index]
            this.step++
        },
        toSymList(s){
